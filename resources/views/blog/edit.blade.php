    ...
    <div class="container mt-6">
        <div class="w-full border rounded p-3 shadow">
            ...
            <form action="" method="post">
                ...
                <input value="{{ $blog->title }}" name="title" required class="border rounded p-2 mt-3 w-full"
                       placeholder="请填写标题" type="text">
                <input value="{{ $blog->img }}" name="img" required class="border rounded p-2 mt-3 w-full"
                       placeholder="填写图片地址" type="text">
                <input value="{{ $blog->tag }} " name="tag" required class="border rounded p-2 mt-3 w-full"
                       placeholder="标签" type="text">
                <select name="cate_id" id="" class="border rounded p-2 my-3 w-full">
                    <option value="text-gray-700">请选择分类</option>
                    @foreach($cates as $v)
                        <option <?php if ($v['id'] == $blog['cate_id']) {
                            echo 'selected';
                        } ?> class="py-2 text-gray-700" value="{{$v['id']}}">{{$v['name']}}</option>
                    @endforeach
                </select>

                <textarea name="body" required class="border rounded p-2 mt-3 w-full"
                          id="editor">{{$blog->body}}</textarea>
